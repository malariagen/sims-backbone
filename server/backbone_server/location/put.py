...
class LocationPut(LocationEdit):
    ...
    def put(self, location_id, location, studies):

        with self._connection:
            with self._connection.cursor() as cursor:
                return self.run_command(cursor, location_id, location, studies)

    def run_command(self, cursor, location_id, location, studies):
        ...
        for (loc_id, latitude, longitude, accuracy, curated_name,
            ...
            existing_location = Location(loc_id, latitude, longitude, accuracy,
                                         curated_name, curation_method, country)

        if not existing_location:
            raise MissingKeyException("Error updating location {}".format(location_id))

        LocationEdit.check_for_duplicate(cursor, location, location_id, studies)
